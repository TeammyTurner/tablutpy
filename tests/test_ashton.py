...
class AshtonBoardTest(unittest.TestCase):
...
class AshtonLegalMovesTest(unittest.TestCase):
    def test_to_and_from_same_cell(self):
        board = ashton.Board()
        legal, _ = board.is_legal((2, 4), (2, 4))
        self.assertFalse(legal)

    def test_to_escape_defender(self):
        board = ashton.Board()
        legal, _ = board.is_legal((2, 4), (2, 0))
        self.assertFalse(legal)

    def test_to_escape_attacker(self):
        board = ashton.Board()
        legal, _ = board.is_legal((0, 3), (0, 2))
        self.assertFalse(legal)

    def test_to_camp_not_attacker(self):
        board = ashton.Board()
        legal, _ = board.is_legal((1, 4), (1, 0))
        self.assertFalse(legal)

    def test_not_orthogonal(self):
        board = ashton.Board()
        legal, _ = board.is_legal((0, 3), (1, 1))
        self.assertFalse(legal)
    ...
    def test_pass_over_checker(self):
        board = ashton.Board()
        board.step((0, 3), (1, 3))
        legal, _ = board.is_legal((1, 3), (1, 5))
        self.assertFalse(legal)

    def test_end_on_checker(self):
        board = ashton.Board()
        board.step((0, 3), (1, 3))
        legal, _ = board.is_legal((1, 3), (1, 4))
        self.assertFalse(legal)
    ...
    def test_attacker_moving_on_camp(self):
        ...
        legal, _ = board.is_legal((4, 0), (3, 0))
    ...
    def test_attacker_returning_on_camp(self):
        ...
        board.step((3, 0), (3, 2))
        # slide checkers in camp
        board.is_legal((4, 1), (3, 1))
        legal, _ = board.is_legal((3, 1), (3, 0))
        self.assertFalse(legal)

    def test_move_backward(self):
        board = ashton.Board()
        legal, _ = board.is_legal((3, 8), (3, 5))
        self.assertTrue(legal)
...
class AshtonCaptureTest(unittest.TestCase):
    ...
    def test_passive_capture_consistency(self):
        ...
        self.assertTrue(isinstance(board.board[3][2].piece, EmptyTile))
